...
p.ce.M1n = sparse((diag(ones(p.Nxn-2,1),+1) - diag(ones(p.Nxn-2,1),-1))/(2*Del_xn));
p.ce.M1s = sparse((diag(ones(p.Nxs-2,1),+1) - diag(ones(p.Nxs-2,1),-1))/(2*Del_xs));
p.ce.M1p = sparse((diag(ones(p.Nxp-2,1),+1) - diag(ones(p.Nxp-2,1),-1))/(2*Del_xp));
...
p.ce.M3n = sparse((-2*diag(ones(p.Nxn-1,1),0) + diag(ones(p.Nxn-2,1),+1) + diag(ones(p.Nxn-2,1),-1))/(Del_xn^2));
p.ce.M3s = sparse((-2*diag(ones(p.Nxs-1,1),0) + diag(ones(p.Nxs-2,1),+1) + diag(ones(p.Nxs-2,1),-1))/(Del_xs^2));
p.ce.M3p = sparse((-2*diag(ones(p.Nxp-1,1),0) + diag(ones(p.Nxp-2,1),+1) + diag(ones(p.Nxp-2,1),-1))/(Del_xp^2));
...
p.ce.M5n = (1-p.t_plus)*p.a_s_n/p.epsilon_e_n * speye(p.Nxn-1);
p.ce.M5p = (1-p.t_plus)*p.a_s_p/p.epsilon_e_p * speye(p.Nxp-1);
